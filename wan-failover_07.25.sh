...
scriptmode ()
{
  if [[ "${mode}" == "menu" ]] &>/dev/null;then
    if tty &>/dev/null;then
      ...
      systembinaries || return 1
      [[ -f "${CONFIGFILE}" ]] &>/dev/null && { setvariables || return 1 ;}
      menu || return 1
    else
      return
    fi
    ...
    flock -x -n 100 || { cronjob delete && { if tty &>/dev/null;then echo -e "${LIGHTRED}***${ALIAS} is already running***${NOCOLOR}";fi && exit ;} ;}
    echo -e "$$" > ${PIDFILE}
    ...
    if [[ "${WANSMODE}" == "lb" ]] &>/dev/null;then
      ...
      return
    elif [[ "${WANSMODE}" == "fo" ]] &>/dev/null;then
      ...
      echo -e "${BOLD}${RED}***ASUS Factory Failback is Enabled***${NOCOLOR}"
      return
    fi
...
systemcheck ()
{
  ...
  if [[ -f "/jffs/configs/profile.add" ]] &>/dev/null && [[ -z "$(grep -w "# Wan-Failover" /jffs/configs/profile.add)" ]] &>/dev/null;then
  ...
  while [[ -z "${systemparameterssync+x}" ]] &>/dev/null || [[ "${systemparameterssync}" == "0" ]] &>/dev/null;do
    ...
    # CODEBASE
    if [[ -z "${CODEBASE+x}" ]] &>/dev/null;then
      CODEBASE="$(nvram get innerver & nvramcheck)"
      [[ -n "${CODEBASE}" ]] &>/dev/null && CODEBASE="$(echo ${CODEBASE:0:7} | tr -d .)" || { logger -p 6 -t "${ALIAS}" "Debug - failed to set CODEBASE" && unset CODEBASE && continue ;}
    fi 
  ...
  while [[ -z "${activesystemsync+x}" ]] &>/dev/null || [[ "${activesystemsync}" == "0" ]] &>/dev/null;do
    if [[ -z "${activesystemsync+x}" ]] &>/dev/null;then
      activesystemsync="0"
    elif [[ "${activesystemsync}" == "1" ]] &>/dev/null;then
      break
    else
      sleep 1
    fi
    ...
    # LEDDISABLE
    # Codebase 3006
    if [[ "${CODEBASE}" == "3006" ]] &>/dev/null;then
      if [[ -z "${LEDDISABLE+x}" ]] &>/dev/null || [[ -z "${zLEDDISABLE+x}" ]] &>/dev/null;then
      led_val="$(nvram get led_val & nvramcheck)"
        if [[ "${led_val}" == "1" ]] &>/dev/null;then
        LEDDISABLE="0"
          zLEDDISABLE="0"
        elif [[ "${led_val}" == "0" ]] &>/dev/null;then
          LEDDISABLE="1"
          zLEDDISABLE="1"
        else
          logger -p 6 -t "${ALIAS}" "Debug - failed to set LEDDISABLE"
          unset LEDDISABLE zLEDDISABLE
          continue
        fi
      else
      ...
      led_val="$(nvram get led_val & nvramcheck)"
        ...
        LEDDISABLE="0"
          ...
          LEDDISABLE="${zLEDDISABLE}"
        fi
        [[ -n "${LEDDISABLE}" ]] &>/dev/null || LEDDISABLE="${zLEDDISABLE}"
      fi
      unset led_val
    # Codebase 3004
    elif [[ "${CODEBASE}" == "3004" ]] &>/dev/null;then
      if [[ -z "${LEDDISABLE+x}" ]] &>/dev/null || [[ -z "${zLEDDISABLE+x}" ]] &>/dev/null;then
...
install ()
{
  ...
  getsystemparameters && echo -e "${GREEN}Successfully acquired System Settings${NOCOLOR}" || { echo -e "${RED}Failed to acquire System Settings${NOCOLOR}" && return 1 ;}

  # Get Global WAN Parameters
  if [[ -z "${globalwansync+x}" ]] &>/dev/null;then
    echo -e "${LIGHTBLUE}Getting WAN Parameters...${NOCOLOR}"
    GETWANMODE="2"
    getwanparameters && echo -e "${GREEN}Successfully acquired WAN Parameters${NOCOLOR}" || { echo -e "${RED}Failed to acquire WAN Parameters${NOCOLOR}" && return 1 ;}
  fi

  # Check for Config File
  if [[ ! -f ${CONFIGFILE} ]] &>/dev/null;then
    echo -e "${LIGHTBLUE}Creating ${CONFIGFILE}...${NOCOLOR}" ; logger -p 5 -t "${ALIAS}" "Install - Creating ${CONFIGFILE}"
    { touch -a ${CONFIGFILE} && chmod 666 ${CONFIGFILE} && setvariables || return 1 ;} \
    && { echo -e "${GREEN}${CONFIGFILE} created successfully.${NOCOLOR}" ; logger -p 5 -t "${ALIAS}" "Install - ${CONFIGFILE} created successfully" ;} \
    || { { echo -e "${RED}${CONFIGFILE} failed to create.${NOCOLOR}" ; logger -p 5 -t "${ALIAS}" "Install - ${CONFIGFILE} failed to create" ;} && return 1 ;}
  else
  ...
  if [[ ! -f "/jffs/scripts/wan-event" ]] &>/dev/null;then
    ...
    || { { echo -e "${RED}wan-event script failed to create.${NOCOLOR}" ; logger -p 5 -t "${ALIAS}" "Install - wan-event script failed to create" ;} && return 1 ;}
  else
  ...
  if [[ -n "$(grep -w "# Wan-Failover" /jffs/scripts/wan-event)" ]] &>/dev/null;then 
    ...
    || { { echo -e "${RED}${ALIAS} failed to add to wan-event.${NOCOLOR}" ; logger -p 5 -t "${ALIAS}" "Install - ${ALIAS} failed to add to wan-event" ;} && return 1 ;}
  fi
  ...
  if [[ ! -f "/jffs/configs/profile.add" ]] &>/dev/null;then
    ...
    || { { echo -e "${RED}/jffs/configs/profile.add failed to be created.${NOCOLOR}" ; logger -p 5 -t "${ALIAS}" "Install - /jffs/configs/profile.add failed to be created" ;} && return 1 ;}
  else
  ...
  if [[ -z "$(grep -w "# Wan-Failover" /jffs/configs/profile.add)" ]] &>/dev/null;then
    ...
    || { { echo -e "${RED}${ALIAS} - Install: Failed to create Alias for ${0} as wan-failover...${NOCOLOR}" && logger -p 5 -t "${ALIAS}" "Install - Failed to create Alias for ${0} as wan-failover" ;} && return 1 ;}
  fi

  # Create Initial Cron Jobs
  echo -e "${LIGHTBLUE}Creating Cron Job...${NOCOLOR}"
  cronjob &>/dev/null && echo -e "${GREEN}Created Cron Job${NOCOLOR}" || { echo -e "${RED}Failed to create Cron Job${NOCOLOR}" && return 1 ;}
  ...
  # Check if ASUS Factory Failover is Enabled
  if [[ "${WANDOGENABLE}" == "1" ]] &>/dev/null;then
    echo -e "${RED}***Warning***  ASUS Factory WAN Failover Enabled.${NOCOLOR}" ; logger -p 3 -t "${ALIAS}" "Install - ***Warning***  ASUS Factory WAN Failover Enabled"
    echo -e "${RED}***Warning***  Disable WAN > Dual WAN > Auto Network Detection > Network Monitoring > Ping ${NOCOLOR}" ; logger -p 3 -t "${ALIAS}" "Install - ***Warning***  Disable WAN > Dual WAN > Auto Network Detection > Network Monitoring > Ping"
  elif [[ "${WANDOGENABLE}" == "0" ]] &>/dev/null;then
    echo -e "${GREEN}ASUS Factory WAN Failover Disabled.${NOCOLOR}" ; logger -p 5 -t "${ALIAS}" "Install - ASUS Factory WAN Failover Disabled"
  fi

  # Check if ASUS Factory Failback is Enabled
  if [[ "${WANSMODE}" == "fb" ]] &>/dev/null;then
    echo -e "${RED}***Warning***  ASUS Factory WAN Failback Enabled.${NOCOLOR}" ; logger -p 3 -t "${ALIAS}" "Install - ***Warning***  ASUS Factory WAN Failback Enabled"
    ...
    echo -e "${GREEN}ASUS Factory Failback Disabled.${NOCOLOR}" ; logger -p 5 -t "${ALIAS}" "Install - ASUS Factory Failback Disabled"
  fi

  # Check if ASUS Factory DNS Probe is Enabled
  if [[ "${DNSPROBE}" == "1" ]] &>/dev/null;then
    echo -e "${RED}***Warning***  ASUS Factory DNS Probe Enabled.${NOCOLOR}" ; logger -p 3 -t "${ALIAS}" "Install - ***Warning***  ASUS Factory DNS Probe Enabled"
    echo -e "${RED}***Warning***  Disable WAN > Dual WAN > Auto Network Detection > Network Monitoring > DNS Query ${NOCOLOR}" ; logger -p 3 -t "${ALIAS}" "Install - ***Warning***  Disable WAN > Dual WAN > Auto Network Detection > Network Monitoring > DNS Query"
  elif [[ "${DNSPROBE}" == "0" ]] &>/dev/null;then
    echo -e "${GREEN}ASUS Factory DNS Probe Disabled.${NOCOLOR}" ; logger -p 5 -t "${ALIAS}" "Install - ASUS Factory DNS Probe Disabled"
  fi
...
uninstall ()
{
  ...
  if [[ "${mode}" == "uninstall" ]] &>/dev/null || [[ "${mode}" == "install" ]] &>/dev/null;then
...
cleanup ()
{
  ...
  # Restore Cron Job if Configured
  if [[ "$(awk -F "=" '$1 == "SCHEDULECRONJOB" {print $2}' ${CONFIGFILE})" == "1" ]] &>/dev/null;then
    cronjob create
  fi
...
update ()
{
  ...
  elif [[ "${DEVMODE}" == "2" ]] &>/dev/null;then
    [[ "${passiveupdate}" == "0" ]] &>/dev/null && echo -e "${RED}***${ALIAS} current Dev Mode does not allow for updates***${NOCOLOR}"
    updateneeded="0"
    return
  fi
...
cronjob ()
{
  ...
  if [[ "${cron_arg1}" == "create" ]] &>/dev/null || { [[ "${SCHEDULECRONJOB}" == "1" ]] &>/dev/null && { [[ "${mode}" == "cron" ]] &>/dev/null || [[ "${mode}" == "install" ]] &>/dev/null || [[ "${mode}" == "restart" ]] &>/dev/null || [[ "${mode}" == "update" ]] &>/dev/null ;} ;};then
  ...
  # Remove Cron Job
  elif [[ "${cron_arg1}" == "delete" ]] &>/dev/null || [[ "${SCHEDULECRONJOB}" == "0" ]] &>/dev/null || [[ "${mode}" == "kill" ]] &>/dev/null || [[ "${mode}" == "uninstall" ]] &>/dev/null;then
...
setvariables ()
{
  ...
  if [[ "${configdefaultssync}" == "0" ]] &>/dev/null;then
    ...
    [[ -n "${PRODUCTID+x}" ]] &>/dev/null || { getsystemparameters || return 1 ;}
    ...
    if [[ -z "$(sed -n '/\bPREFERRED_WAN=\b/p' "${CONFIGFILE}")" ]] &>/dev/null;then
      logger -p 6 -t "${ALIAS}" "Debug - Setting PREFERRED_WAN Default: AUTO"
      echo -e "PREFERRED_WAN=AUTO" >> ${CONFIGFILE}
    fi
    ...
    if [[ -z "$(sed -n '/\bPINGTIMELOGGING=\b/p' "${CONFIGFILE}")" ]] &>/dev/null;then
      logger -p 6 -t "${ALIAS}" "Debug - Creating PINGTIMELOGGING Default: Enabled"
      echo -e "PINGTIMELOGGING=1" >> ${CONFIGFILE}
    fi
    ...
    if [[ -z "$(sed -n '/\bFAILOVERTIMEOUT=\b/p' "${CONFIGFILE}")" ]] &>/dev/null;then
...
config ()
{
  ...
  printf "  (5)  Configure Ping Time Logging     Ping Time Logging: " && { [[ "${PINGTIMELOGGING}" == "1" ]] &>/dev/null && printf "${GREEN}Enabled${NOCOLOR}" || printf "${RED}Disabled${NOCOLOR}" ;} && printf "\n"
  printf "  (6)  Configure Ping Time Min         Ping Time Minimum: ${GREEN}${PINGTIMEMIN}ms${NOCOLOR}\n"
  printf "  (7)  Configure Ping Time Max         Ping Time Maximum: ${RED}${PINGTIMEMAX}ms${NOCOLOR}\n"

  printf "\n  ${BOLD}QoS Settings:${NOCOLOR}\n"
  printf "  (8)  Configure WAN0 QoS              WAN0 QoS: " && { [[ "${WAN0_QOS_ENABLE}" == "1" ]] &>/dev/null && printf "${GREEN}Enabled${NOCOLOR}" || printf "${RED}Disabled${NOCOLOR}" ;} && printf "\n"
  if [[ "${WAN0_QOS_ENABLE}" == "1" ]] &>/dev/null || [[ "${DEVMODE}" -ge "1" ]] &>/dev/null;then
    printf "    (8a) Configure Download Speed       - Download Speed: ${LIGHTBLUE}${wan0qosibw}${NOCOLOR}\n"
    printf "    (8b) Configure Upload Speed         - Upload Speed: ${LIGHTBLUE}${wan0qosobw}${NOCOLOR}\n"
    printf "    (8c) Configure Packet Overhead      - Packet Overhead: ${LIGHTBLUE}${WAN0_QOS_OVERHEAD} Bytes${NOCOLOR}\n"
    printf "    (8d) Configure ATM Mode             - ATM Mode: " && { [[ "${WAN0_QOS_ATM}" == "1" ]] &>/dev/null && printf "${GREEN}Enabled${NOCOLOR}" || printf "${RED}Disabled${NOCOLOR}" ;} && printf "\n"
  fi

  printf "  (9)  Configure WAN1 QoS              WAN1 QoS: " && { [[ "${WAN1_QOS_ENABLE}" == "1" ]] &>/dev/null && printf "${GREEN}Enabled${NOCOLOR}" || printf "${RED}Disabled${NOCOLOR}" ;} && printf "\n"
  if [[ "${WAN1_QOS_ENABLE}" == "1" ]] &>/dev/null || [[ "${DEVMODE}" -ge "1" ]] &>/dev/null;then
    printf "    (9a) Configure Download Speed       - Download Speed: ${LIGHTBLUE}${wan1qosibw}${NOCOLOR}\n"
    printf "    (9b) Configure Upload Speed         - Upload Speed: ${LIGHTBLUE}${wan1qosobw}${NOCOLOR}\n"
    printf "    (9c) Configure Packet Overhead      - Packet Overhead: ${LIGHTBLUE}${WAN1_QOS_OVERHEAD} Bytes${NOCOLOR}\n"
    printf "    (9d) Configure ATM Mode             - ATM Mode: " && { [[ "${WAN1_QOS_ATM}" == "1" ]] &>/dev/null && printf "${GREEN}Enabled${NOCOLOR}" || printf "${RED}Disabled${NOCOLOR}" ;} && printf "\n"
  fi

  printf "\n  ${BOLD}Optional Settings:${NOCOLOR}\n"
  printf "  (10) Configure Packet Loss Logging   Packet Loss Logging: " && { [[ "${PACKETLOSSLOGGING}" == "1" ]] &>/dev/null && printf "${GREEN}Enabled${NOCOLOR}" || printf "${RED}Disabled${NOCOLOR}" ;} && printf "\n"
  printf "  (11) Configure Boot Delay Timer      Boot Delay Timer: ${LIGHTBLUE}${BOOTDELAYTIMER} Seconds${NOCOLOR}\n"
  printf "  (12) Configure Email Notifications   Email Notifications: " && { [[ "${SENDEMAIL}" == "1" ]] &>/dev/null && printf "${GREEN}Enabled${NOCOLOR}" || printf "${RED}Disabled${NOCOLOR}" ;} && printf "\n"
  printf "  (13) Configure WAN0 Packet Size      WAN0 Packet Size: ${LIGHTBLUE}${WAN0PACKETSIZE} Bytes${NOCOLOR}\n"
  printf "  (14) Configure WAN1 Packet Size      WAN1 Packet Size: ${LIGHTBLUE}${WAN1PACKETSIZE} Bytes${NOCOLOR}\n"
  printf "  (15) Configure NVRAM Checks          NVRAM Checks: " && { [[ "${CHECKNVRAM}" == "1" ]] &>/dev/null && printf "${GREEN}Enabled${NOCOLOR}" || printf "${RED}Disabled${NOCOLOR}" ;} && printf "\n"
  printf "  (16) Configure Dev Mode              Dev Mode: " && { [[ "${DEVMODE}" -ge "1" ]] &>/dev/null && printf "${GREEN}Enabled${NOCOLOR}" || printf "Disabled" ;} && printf "\n"
  printf "  (17) Configure Custom Log Path       Custom Log Path: " && { [[ -n "${CUSTOMLOGPATH}" ]] &>/dev/null && printf "${LIGHTBLUE}${CUSTOMLOGPATH}${NOCOLOR}" || printf "${RED}Disabled${NOCOLOR}" ;} && printf "\n"
  printf "  (18) Configure WAN0 Web GUI          WAN0 Web GUI IP Address: " && { [[ -n "${WAN0WEBGUI}" ]] &>/dev/null && printf "${LIGHTBLUE}${WAN0WEBGUI}${NOCOLOR}" || printf "${RED}Disabled${NOCOLOR}" ;} && printf "\n"
  printf "  (19) Configure WAN1 Web GUI          WAN1 Web GUI IP Address: " && { [[ -n "${WAN1WEBGUI}" ]] &>/dev/null && printf "${LIGHTBLUE}${WAN1WEBGUI}${NOCOLOR}" || printf "${RED}Disabled${NOCOLOR}" ;} && printf "\n"

  printf "\n  ${BOLD}Advanced Settings:${NOCOLOR}  ${RED}***Recommended to leave default unless necessary to change***${NOCOLOR}\n"
  printf "  (20) Configure WAN0 Target Priority  WAN0 Target Priority: ${LIGHTBLUE}${WAN0TARGETRULEPRIORITY}${NOCOLOR}\n"
  printf "  (21) Configure WAN1 Target Priority  WAN1 Target Priority: ${LIGHTBLUE}${WAN1TARGETRULEPRIORITY}${NOCOLOR}\n"
  printf "  (22) Configure Recursive Ping Check  Recursive Ping Check: ${LIGHTBLUE}${RECURSIVEPINGCHECK}${NOCOLOR}\n"
  printf "  (23) Configure WAN Disabled Timer    WAN Disabled Timer: ${LIGHTBLUE}${WANDISABLEDSLEEPTIMER} Seconds${NOCOLOR}\n"
  printf "  (24) Configure Email Boot Delay      Email Boot Delay: ${LIGHTBLUE}${SKIPEMAILSYSTEMUPTIME} Seconds${NOCOLOR}\n"
  printf "  (25) Configure Email Timeout         Email Timeout: ${LIGHTBLUE}${EMAILTIMEOUT} Seconds${NOCOLOR}\n"
  printf "  (26) Configure Cron Job              Cron Job: " && { [[ "${SCHEDULECRONJOB}" == "1" ]] &>/dev/null && printf "${GREEN}Enabled${NOCOLOR}" || printf "${RED}Disabled${NOCOLOR}" ;} && printf "\n"
  printf "  (27) Configure Status Check          Status Check Interval: ${LIGHTBLUE}${STATUSCHECK} Seconds${NOCOLOR}\n"
  printf "  (28) Configure Process Priority      Process Priority: " && { { [[ "${PROCESSPRIORITY}" == "0" ]] && printf "${LIGHTBLUE}Normal${NOCOLOR}" ;} || { [[ "${PROCESSPRIORITY}" == "-20" ]] && printf "${LIGHTCYAN}Real Time${NOCOLOR}" ;} || { [[ "${PROCESSPRIORITY}" == "-10" ]] && printf "${LIGHTMAGENTA}High${NOCOLOR}" ;} || { [[ "${PROCESSPRIORITY}" == "10" ]] && printf "${LIGHTYELLOW}Low${NOCOLOR}" ;} || { [[ "${PROCESSPRIORITY}" == "20" ]] && printf "${LIGHTRED}Lowest${NOCOLOR}" ;} || printf "${LIGHTGRAY}${PROCESSPRIORITY}${NOCOLOR}" ;} && printf "\n"
  printf "  (29) Configure Failover Block IPV6   Failover Block IPV6: " && { [[ "${FOBLOCKIPV6}" == "1" ]] &>/dev/null && printf "${GREEN}Enabled${NOCOLOR}" || printf "${RED}Disabled${NOCOLOR}" ;} && printf "\n"
  ...
  printf "  (33) Configure Preferred WAN         Preferred WAN: " && { [[ "${PREFERRED_WAN}" == "AUTO" ]] &>/dev/null && printf "${LIGHTBLUE}AUTO${NOCOLOR}" || printf "${LIGHTBLUE}${PREFERRED_WAN}${NOCOLOR}" ;} && printf "\n"
  ...
  case ${configinput} in
    ...
    '5')      # PINGTIMELOGGING
    while true &>/dev/null;do
      read -p "Do you want to enable Ping Time Logging? ***Enter Y for Yes or N for No***" yn
      yn=${yn//[$'\t\r\n']/}
      case ${yn} in
        [Yy]* ) SETPINGTIMELOGGING="1"; break;;
        [Nn]* ) SETPINGTIMELOGGING="0"; break;;
        * ) echo -e "${RED}Invalid Selection!!! ***Enter Y for Yes or N for No***${NOCOLOR}"
      esac
    done
    NEWVARIABLES="${NEWVARIABLES} PINGTIMELOGGING=|${SETPINGTIMELOGGING}"
    [[ "${RESTARTREQUIRED}" == "0" ]] &>/dev/null && RESTARTREQUIRED="1"
    ;;
    '6')      # PINGTIMEMIN
    while true &>/dev/null;do  
    ...
    '16')      # DEVMODE
    while true &>/dev/null;do
      read -p "Configure Developer Mode - 2 for Advanced Dev Mode (Update Channels Disabled), 1 for Dev Mode (Beta Update Channel), 0 for Standard Mode (Production Update Channel): " value
      value=${value//[$'\t\r\n']/}
      case ${value} in
        0 ) SETDEVMODE="0"; break;;
        1 ) SETDEVMODE="1"; break;;
        2 ) SETDEVMODE="2"; break;;
        * ) echo -e "${RED}Invalid Selection!!! ***Select a Value between 0 and 2***${NOCOLOR}"
      esac
    done
  ...
  NEWVARIABLES="${NEWVARIABLES} FAILOVERTIMEOUT=|${SETFAILOVERTIMEOUT}"
    ...
    '33')      # PREFERRED_WAN
      while true &>/dev/null;do
        read -p "Configure Preferred WAN (AUTO/WAN0/WAN1) - AUTO allows normal failover/failback, WAN0/WAN1 forces specific WAN as primary when available: " value
        value=${value//[$'\t\r\n']/}
        case ${value} in
          AUTO|WAN0|WAN1 ) SETPREFERRED_WAN="${value}"; break;;
          * ) echo -e "${RED}Invalid Selection!!! ***Enter AUTO, WAN0, or WAN1***${NOCOLOR}"
        esac
      done
      NEWVARIABLES="${NEWVARIABLES} PREFERRED_WAN=|${SETPREFERRED_WAN}"
      [[ "${RESTARTREQUIRED}" == "0" ]] &>/dev/null && RESTARTREQUIRED="1"
      ;;
    '34')      # LBRULEPRIORITY
    while true &>/dev/null;do
...
wanstatus ()
{
  ...
  # Check if ASUS Factory WAN Failover is Enabled
    ...
    wandisabled
  # Check if ASUS Factory WAN Failback is Enabled
  elif [[ "${WANSMODE}" == "fb" ]] &>/dev/null;then
    logger -p 2 -st "${ALIAS}" "WAN Status - ASUS Factory Failback: Enabled"
    wandisabled
  # Check if ASUS Factory DNS Probe is Enabled
  elif [[ "${DNSPROBE}" != "0" ]] &>/dev/null;then
    logger -p 2 -st "${ALIAS}" "WAN Status - ASUS Factory DNS Probe: Enabled"
    wandisabled
  # Check if WAN Interfaces are Enabled and Connected
  else
    # Check IP Rules and IPTables Rules
    checkiprules || return 1
    logger -p 4 "4" "4"
    for WANPREFIX in ${WANPREFIXES};do
      # Getting WAN Parameters
      GETWANMODE="1"
      ...
      # Check if WAN is Enabled
        ...
        while [[ "${i}" -le "${RECURSIVEPINGCHECK}" ]] &>/dev/null;do
          ...
          if [[ -z "$(ip route list ${TARGET} via ${GATEWAY} dev ${GWIFNAME} table main metric 1)" ]] &>/dev/null;then
              logger -p 5 -t "${ALIAS}" "WAN Status - Adding route for ${TARGET} via ${GATEWAY} dev ${GWIFNAME}"
              ip route add ${TARGET} via ${GATEWAY} dev ${GWIFNAME} table main metric 1 \
  ...
  ## Check for quick WAN preference change
  #if [[ "${READYSTATE}" == "1" ]] &>/dev/null;then
  #  quickwanswitch || return 1
  #fi
  ...
  if [[ "${mode}" == "initiate" ]] &>/dev/null;then
    ...
    # Determine which function to use based on Secondary WAN
        ...
        # Check if WAN1 is preferred - skip failback monitor
        PREFERRED_WAN="$(awk -F "=" '$1 == "PREFERRED_WAN" {print $2}' "${CONFIGFILE}")"
        [[ -z "${PREFERRED_WAN}" ]] &>/dev/null && PREFERRED_WAN="AUTO"
        
        if [[ "${PREFERRED_WAN}" == "WAN1" ]] &>/dev/null;then
            logger -p 6 -t "${ALIAS}" "Debug - WAN1 is preferred, staying on WAN1"
            # Continue monitoring without failback
            while [[ "${WANSMODE}" == "fo" ]] &>/dev/null && [[ "${WAN1PRIMARY}" == "1" ]] &>/dev/null;do
            # Get Active WAN Parameters
            GETWANMODE="3"
            getwanparameters || return 1
            ...
            # Check if we need to switch back to AUTO mode
            PREFERRED_WAN="$(awk -F "=" '$1 == "PREFERRED_WAN" {print $2}' "${CONFIGFILE}")"
            if [[ "${PREFERRED_WAN}" != "WAN1" ]] &>/dev/null;then
                break
            fi
            ...
            if [[ "${WAN1PACKETLOSS}" == "100%" ]] &>/dev/null || [[ "${WAN1STATUS}" != "CONNECTED" ]] &>/dev/null;then
                if [[ "${WAN0STATUS}" == "CONNECTED" ]] &>/dev/null;then
                logger -p 1 -st "${ALIAS}" "WAN Status - WAN1 failed, switching to WAN0"
                failover
                break
                fi
            fi
...
checkroutingtable ()
{
  ...
  for WANPREFIX in ${WANPREFIXES};do
    ...
    if [[ -z "$(ip route list ${TARGET} via ${GATEWAY} dev ${GWIFNAME} table ${TABLE} metric 1)" ]] &>/dev/null;then
      ...
      ip route add ${TARGET} via ${GATEWAY} dev ${GWIFNAME} table ${TABLE} metric 1 \
...
getwanparameters ()
{
  ...
  if [[ "${GETWANMODE}" == "1" ]] &>/dev/null;then
    ...
    while [[ -z "${wansync+x}" ]] &>/dev/null || [[ "${wansync}" == "0" ]] &>/dev/null;do
      if [[ -z "${wansync+x}" ]] &>/dev/null;then
        wansync="0"
      elif [[ "${wansync}" == "1" ]] &>/dev/null;then
        break
      else
        sleep 1
      fi
      ...
      if [[ "${WANPREFIX}" == "${WAN0}" ]] &>/dev/null;then
        ...
        if [[ -n "${WAN0DUALWANDEV+x}" ]] &>/dev/null;then
          ...
          dualwandevtmp="$(nvram get wans_dualwan & nvramcheck)"
          DUALWANDEV="$(echo "${dualwandevtmp}" | awk '$0 != "" {print $1}')"
          [[ -n "${DUALWANDEV}" ]] &>/dev/null && unset dualwandevtmp || { logger -p 6 -t "${ALIAS}" "Debug - failed to set DUALWANDEV for ${WANPREFIX}" && unset DUALWANDEV && continue ;}
        fi
        ...
        if [[ -n "${WAN0ROUTETABLE+x}" ]] &>/dev/null;then
          ...
          GETWANMODE="2"
          getwanparameters && GETWANMODE="1" || return 1
          TABLE="${WAN0ROUTETABLE}"
        fi
        ...
        if [[ -n "${WAN1DUALWANDEV+x}" ]] &>/dev/null;then
          ...
          dualwandevtmp="$(nvram get wans_dualwan & nvramcheck)"
          DUALWANDEV="$(echo "${dualwandevtmp}" | awk '$0 != "" {print $2}')"
          [[ -n "${DUALWANDEV}" ]] &>/dev/null && unset dualwandevtmp || { logger -p 6 -t "${ALIAS}" "Debug - failed to set DUALWANDEV for ${WANPREFIX}" && unset DUALWANDEV && continue ;}
        fi
  ...
  # Get Global WAN Parameters
    ...
    while [[ -z "${globalwansync+x}" ]] &>/dev/null || [[ "${globalwansync}" == "0" ]] &>/dev/null;do
      if [[ -z "${globalwansync+x}" ]] &>/dev/null;then
        globalwansync="0"
      elif [[ "${globalwansync}" == "1" ]] &>/dev/null;then
        break
      else
        sleep 1
      fi
      ...
      if [[ -z "${WANSDUALWANENABLE+x}" ]] &>/dev/null;then
        wansdualwanenabletmp="$(nvram get wans_dualwan & nvramcheck)"
        [[ -n "$(echo "${wansdualwanenabletmp}" | awk '{if ($0 != "" && $2 != "none") {print $2}}')" ]] &>/dev/null && WANSDUALWANENABLE="1" || WANSDUALWANENABLE="0"
        [[ -n "${WANSDUALWANENABLE}" ]] &>/dev/null && unset wansdualwanenabletmp || { logger -p 6 -t "${ALIAS}" "Debug - failed to set WANSDUALWANENABLE" && unset WANSDUALWANENABLE && continue ;}
      fi
      ...
      # DNSPROBE
      if [[ -z "${DNSPROBE+x}" ]] &>/dev/null;then
        DNSPROBE="$(nvram get dns_probe & nvramcheck)"
        [[ -n "${DNSPROBE}" ]] &>/dev/null || { logger -p 6 -t "${ALIAS}" "Debug - failed to set DNSPROBE" && unset DNSPROBE && continue ;}
      fi
      ...
      if [[ -z "${WAN0DUALWANDEV+x}" ]] &>/dev/null;then
        wan0dualwandevtmp="$(nvram get wans_dualwan & nvramcheck)"
        WAN0DUALWANDEV="$(echo "${wan0dualwandevtmp}" | awk '$0 != "" {print $1}')"
        [[ -n "${WAN0DUALWANDEV}" ]] &>/dev/null && unset wan0dualwandevtmp || { logger -p 6 -t "${ALIAS}" "Debug - failed to set WAN0DUALWANDEV" && unset WAN0DUALWANDEV && continue ;}
      fi
      ...
      if [[ -z "${WAN1DUALWANDEV+x}" ]] &>/dev/null;then
        wan1dualwandevtmp="$(nvram get wans_dualwan & nvramcheck)"
        WAN1DUALWANDEV="$(echo "${wan1dualwandevtmp}" | awk '$0 != "" {print $2}')"
        [[ -n "${WAN1DUALWANDEV}" ]] &>/dev/null && unset wan1dualwandevtmp || { logger -p 6 -t "${ALIAS}" "Debug - failed to set WAN1DUALWANDEV" && unset WAN1DUALWANDEV && continue ;}
      fi
...
pingtargets ()
{
  ...
  while [[ "${i}" -le "${RECURSIVEPINGCHECK}" ]] &>/dev/null;do
    ...
    # Ping Time Logging if Enabled
    if [[ "${PINGTIMELOGGING}" == "1" ]] &>/dev/null;then
...
failover ()
{
  ...
  restartservices || return 1

  flushconntrack || return 1

  checkiprules || return 1

  [[ "${email}" == "1" ]] &>/dev/null && { sendemail && email="0" || return 1 ;}
  return
}
...
lbmonitor ()
{
  ...
  while { [[ "${WANSMODE}" == "lb" ]] &>/dev/null && [[ "${WAN0ENABLE}" == "1" ]] &>/dev/null && [[ "${WAN1ENABLE}" == "1" ]] &>/dev/null ;};do
    ...
    # Check for quick WAN preference change every 30 iterations
    if [[ "$((${lbmonitorloop} % 30))" == "0" ]] &>/dev/null;then
      quickwanswitch || return 1
    fi
...
wan0failovermonitor ()
{
  ...
  logger -p 6 -t "${ALIAS}!" "wan0failOVERmonitor"
  # Get Global WAN Parameters
  if [[ -z "${globalwansync+x}" ]] &>/dev/null;then
    GETWANMODE="2"
    getwanparameters || return 1
  ...
  while [[ "${WANSMODE}" == "fo" ]] &>/dev/null && [[ "${WAN0PRIMARY}" == "1" ]] &>/dev/null;do
...
wan0failbackmonitor ()
{
  logger -p 6 -t "${ALIAS}!" "wan0failbackmonitor"
  logger -p 6 -t "${ALIAS}" "Debug - Function: wan0failbackmonitor"

  # Get PREFERRED_WAN setting
  PREFERRED_WAN="$(awk -F "=" '$1 == "PREFERRED_WAN" {print $2}' "${CONFIGFILE}")"
  [[ -z "${PREFERRED_WAN}" ]] &>/dev/null && PREFERRED_WAN="AUTO"

  # Skip failback if WAN1 is preferred
  if [[ "${PREFERRED_WAN}" == "WAN1" ]] &>/dev/null;then
    logger -p 4 -st "${ALIAS}" "WAN0 Failback Monitor - Skipped, WAN1 is preferred primary"
    return
  fi
  ...
  while \
    ...
    || { [[ "${WAN1ENABLE}" == "1" ]] &>/dev/null && [[ "${WAN1STATE}" == "3" ]] &>/dev/null ;};then
      ...
      [[ "${WAN1STATE}" == "3" ]] &>/dev/null && logger -p 3 -st "${ALIAS}" "WAN Failover Disabled - ${WAN1} is connected with State: ${WAN1STATE}"
        unset wandisabledloop
        wanstatus
    # Check if we're waiting for preferred WAN to come back
    elif [[ -n "${PREFERRED_WAN+x}" ]] &>/dev/null && [[ "${PREFERRED_WAN}" != "AUTO" ]] &>/dev/null;then
    PREFERRED_WAN="$(awk -F "=" '$1 == "PREFERRED_WAN" {print $2}' "${CONFIGFILE}")"
    
    if [[ "${PREFERRED_WAN}" == "WAN1" ]] &>/dev/null;then
        # Check if WAN1 is back online
        if [[ "${WAN1ENABLE}" == "1" ]] &>/dev/null && [[ "${WAN1STATE}" == "2" ]] &>/dev/null && [[ "${WAN1AUXSTATE}" == "0" ]] &>/dev/null;then
        if [[ "${WAN1IPADDR}" != "0.0.0.0" ]] &>/dev/null && [[ -n "${WAN1IPADDR}" ]] &>/dev/null && [[ "${WAN1GATEWAY}" != "0.0.0.0" ]] &>/dev/null && [[ -n "${WAN1GATEWAY}" ]] &>/dev/null;then
            logger -p 4 -st "${ALIAS}" "WAN Failover Disabled - Preferred WAN1 is back online"
            unset wandisabledloop
            # Force switch to WAN1
            if [[ "${WAN0PRIMARY}" == "1" ]] &>/dev/null;then
            email="1"
            failover && email="0"
            fi
    ...
    elif [[ "${PREFERRED_WAN}" == "WAN0" ]] &>/dev/null;then
        # Check if WAN0 is back online
        if [[ "${WAN0ENABLE}" == "1" ]] &>/dev/null && [[ "${WAN0STATE}" == "2" ]] &>/dev/null && [[ "${WAN0AUXSTATE}" == "0" ]] &>/dev/null;then
        if [[ "${WAN0IPADDR}" != "0.0.0.0" ]] &>/dev/null && [[ -n "${WAN0IPADDR}" ]] &>/dev/null && [[ "${WAN0GATEWAY}" != "0.0.0.0" ]] &>/dev/null && [[ -n "${WAN0GATEWAY}" ]] &>/dev/null;then
            logger -p 4 -st "${ALIAS}" "WAN Failover Disabled - Preferred WAN0 is back online"
            unset wandisabledloop
            # Force switch to WAN0
            if [[ "${WAN1PRIMARY}" == "1" ]] &>/dev/null;then
            email="1"
            failover && email="0"
            fi
    ...
    # Continue monitoring for preferred WAN
    [[ "${wandisabledloop}" == "1" ]] &>/dev/null && logger -p 3 -st "${ALIAS}" "WAN Failover Disabled - Waiting for preferred ${PREFERRED_WAN} to become available"
    wandisabledloop="$((${wandisabledloop}+1))"
    sleep ${WANDISABLEDSLEEPTIMER}
    continue
...
switchwan ()
{
  logger -p 6 -t "${ALIAS}" "Debug - Function: switchwan"
    ...
    # Initialize variables to prevent unset errors
    [[ -z "${STATUS+x}" ]] &>/dev/null && STATUS=""
    [[ -z "${WAN0STATUS+x}" ]] &>/dev/null && WAN0STATUS=""
    [[ -z "${WAN1STATUS+x}" ]] &>/dev/null && WAN1STATUS=""

  # Verify Failover Mode
  if [[ "${WANSMODE}" == "lb" ]] &>/dev/null;then
    return
  fi
    ...
    WAN0PRIMARY="$(nvram get wan0_primary)"
    WAN1PRIMARY="$(nvram get wan1_primary)"

    # Prevent switching if we're already on preferred WAN
    if [[ "${PREFERRED_WAN}" == "WAN1" ]] &>/dev/null && [[ "${WAN1PRIMARY}" == "1" ]] &>/dev/null;then
        logger -p 6 -t "${ALIAS}" "Debug - Already on preferred WAN1, skip switch"
        return
    elif [[ "${PREFERRED_WAN}" == "WAN0" ]] &>/dev/null && [[ "${WAN0PRIMARY}" == "1" ]] &>/dev/null;then
        logger -p 6 -t "${ALIAS}" "Debug - Already on preferred WAN0, skip switch"
        return
    fi


  # Capture Failover Timeout
  if [[ -z "${FAILOVERTIMEOUT+x}" ]] &>/dev/null;then
    ...
    [[ -z "${FAILOVERTIMEOUT}" ]] &>/dev/null && FAILOVERTIMEOUT="30"
  fi
  ...
  # Set SWITCHPRIMARY Flag
  [[ -z "${SWITCHPRIMARY+x}" ]] &>/dev/null && SWITCHPRIMARY="1"

  # Determine Primary WAN based on PREFERRED_WAN setting
  if [[ "${PREFERRED_WAN}" == "WAN1" ]] &>/dev/null;then
    # Force WAN1 as primary if available
    if [[ "$(nvram get wan1_enable & nvramcheck)" == "1" ]] &>/dev/null && \
      [[ "$(nvram get wan1_state_t & nvramcheck)" == "2" ]] &>/dev/null && \
      [[ "$(nvram get wan1_auxstate_t & nvramcheck)" == "0" ]] &>/dev/null;then
      ACTIVEWAN="wan1"
      INACTIVEWAN="wan0"
      logger -p 5 -st "${ALIAS}" "Switch WAN - WAN1 set as preferred primary"
    else
      # Fallback to WAN0 if WAN1 not available
      ACTIVEWAN="wan0"
      INACTIVEWAN="wan1"
      logger -p 5 -st "${ALIAS}" "Switch WAN - WAN1 preferred but not available, using WAN0"
    fi
  elif [[ "${PREFERRED_WAN}" == "WAN0" ]] &>/dev/null;then
    # Force WAN0 as primary
    ACTIVEWAN="wan0"
    INACTIVEWAN="wan1"
    logger -p 5 -st "${ALIAS}" "Switch WAN - WAN0 set as preferred primary"
  else
  ...
  SWITCHTIMEOUT="$(($(awk -F "." '{print $1}' "/proc/uptime")+${FAILOVERTIMEOUT}))"
  ...
  # Perform Failover until complete or timeout
  ...
  && { [[ "$(nvram get ${ACTIVEWAN}_ipaddr & nvramcheck)" == "$(nvram get wan_ipaddr & nvramcheck)" ]] &>/dev/null && [[ "$(nvram get ${ACTIVEWAN}_gateway & nvramcheck)" == "$(nvram get wan_gateway & nvramcheck)" ]] &>/dev/null && [[ "$(nvram get ${ACTIVEWAN}_gw_ifname & nvramcheck)" == "$(nvram get wan_gw_ifname & nvramcheck)" ]] &>/dev/null ;};do
    ...
    if [[ "$(nvram get ${ACTIVEWAN}_primary & nvramcheck)" != "1" ]] &>/dev/null && [[ "$(nvram get ${INACTIVEWAN}_primary & nvramcheck)" != "0" ]] &>/dev/null;then
      ...
      logger -p 6 -t "${ALIAS}" "Debug - ${ACTIVEWAN} Primary Status: $(nvram get ${ACTIVEWAN}_primary & nvramcheck)"
      logger -p 6 -t "${ALIAS}" "Debug - ${INACTIVEWAN} Primary Status: $(nvram get ${INACTIVEWAN}_primary & nvramcheck)"
    fi
    ...
    for WANPREFIX in ${WANPREFIXES};do
      if [[ "${ACTIVEWAN}" != "${WANPREFIX}" ]] &>/dev/null;then
        logger -p 6 -t "${ALIAS}" "Debug - Bypassing QoS Settings configuration for ${WANPREFIX}"
        continue
      elif [[ "${ACTIVEWAN}" == "${WANPREFIX}" ]] &>/dev/null;then
        logger -p 6 -t "${ALIAS}" "Debug - Checking QoS Settings configuration for ${WANPREFIX}"
        GETWANMODE="1"
        getwanparameters || return 1
  ...
  for WANPREFIX in ${WANPREFIXES};do
    ...
    if { [[ "${STATUS}" == "CONNECTED" ]] &>/dev/null && [[ "${WANSMODE}" == "lb" ]] &>/dev/null ;} || { [[ "${WANSMODE}" != "lb" ]] &>/dev/null && [[ "${PRIMARY}" == "1" ]] &>/dev/null ;};then
      ...
      # Change Automatic ISP DNS Settings
        ...
        if [[ "$(nvram get wan_dns & nvramcheck)" != "${DNS}" ]] &>/dev/null && { [[ "${WANSMODE}" != "lb" ]] &>/dev/null && [[ "${PRIMARY}" == "1" ]] &>/dev/null ;};then
...
restartservices ()
{
  ...
  # Check if LEDs are Disabled for Codebase 3004
  if [[ "${CODEBASE}" == "3004" ]]  &>/dev/null && [[ "${RESTARTSERVICESMODE}" == "1" ]] &>/dev/null && [[ "${LEDDISABLE}" == "0" ]] &>/dev/null;then
  ...
  while [[ "${i}" -le "5" ]] &>/dev/null;do
    i="$((${i}+1))"
    if [[ -s "/etc/wg/wgc${i}_status" ]] &>/dev/null;then
      WGVPNS="${WGVPNS} ${i}"
    fi
  done
...
sendemail ()
{
  ...
  elif [[ "$(awk -F "." '{print $1}' "/proc/uptime")" -le "$((${SKIPEMAILSYSTEMUPTIME}+${BOOTDELAYTIMER}))" ]] &>/dev/null;then
...
quickwanswitch ()
{
logger -p 6 -t "${ALIAS}" "Debug - Function: quickwanswitch"

# Check if already switching
[[ -n "${SWITCHING+x}" ]] &>/dev/null && return

# Get current preferred WAN from NVRAM
QUICK_PREFERRED="$(nvram get wan_failover_preferred & nvramcheck)"
[[ -z "${QUICK_PREFERRED}" ]] &>/dev/null && QUICK_PREFERRED="AUTO"

# Get current preferred from config
CURRENT_PREFERRED="$(awk -F "=" '$1 == "PREFERRED_WAN" {print $2}' "${CONFIGFILE}")"
[[ -z "${CURRENT_PREFERRED}" ]] &>/dev/null && CURRENT_PREFERRED="AUTO"

# Check if preference changed
if [[ "${QUICK_PREFERRED}" != "${CURRENT_PREFERRED}" ]] &>/dev/null;then
  logger -p 4 -st "${ALIAS}" "Quick Switch - Preferred WAN changed from ${CURRENT_PREFERRED} to ${QUICK_PREFERRED}"
  
  # Set switching flag
  SWITCHING="1"
  
  # Update configuration file
  sed -i "s/^PREFERRED_WAN=.*/PREFERRED_WAN=${QUICK_PREFERRED}/" "${CONFIGFILE}"
  
  # Force immediate action based on new preference
  if [[ "${QUICK_PREFERRED}" == "WAN1" ]] &>/dev/null;then
    if [[ "${WAN0PRIMARY}" == "1" ]] &>/dev/null && [[ "${WAN1STATUS}" == "CONNECTED" ]] &>/dev/null;then
      logger -p 1 -st "${ALIAS}" "Quick Switch - Forcing switch to preferred WAN1"
      # Set flags to force switch
      WAN0STATUS="DISCONNECTED"
      WANSTATUSMODE="2"
      setwanstatus
      email="1"
  ...
  elif [[ "${QUICK_PREFERRED}" == "WAN0" ]] &>/dev/null;then
    if [[ "${WAN1PRIMARY}" == "1" ]] &>/dev/null && [[ "${WAN0STATUS}" == "CONNECTED" ]] &>/dev/null;then
      logger -p 1 -st "${ALIAS}" "Quick Switch - Forcing switch to preferred WAN0"
      # Set flags to force switch
      WAN1STATUS="DISCONNECTED" 
      WANSTATUSMODE="2"
      setwanstatus
  ...
  elif [[ "${QUICK_PREFERRED}" == "AUTO" ]] &>/dev/null;then
    logger -p 4 -st "${ALIAS}" "Quick Switch - Returning to AUTO mode"
    # Let normal failover logic take over
  fi
...
statusconsole ()
{
  ...
  passiveupdate="1"
  update && unset passiveupdate || return 1
  ...
  while true &>/dev/null;do
    ...
    if [[ -n "$(pidof AdGuardHome)" ]] &>/dev/null || { [[ -f "/opt/etc/AdGuardHome/.config" ]] &>/dev/null && [[ -n "$(awk -F "=" '/ADGUARD_LOCAL/ {print $2}' "/opt/etc/AdGuardHome/.config" | sed -e 's/^"//' -e 's/"$//' | grep -w ^"YES")" ]] &>/dev/null ;};then
      ...
      if [[ -n "${ACTIVEDNSSERVERS}" ]] &>/dev/null || [[ "${DEVMODE}" -ge "1" ]] &>/dev/null;then
        printf "${BOLD}${UNDERLINE}Active DNS Servers:${NOCOLOR}\n"
    ...
    if [[ "${IPV6SERVICE}" != "disabled" ]] &>/dev/null || [[ "${DEVMODE}" -ge "1" ]] &>/dev/null;then
      printf "\n"
